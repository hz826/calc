...
void Listener::calc() {
    ...
    try {
        Number numerical_ans = Calc<Number>(ctext.toStdString()).get();
        Expr symbolic_ans = Calc<Expr>(ctext.toStdString()).get();

        //QString nans = QString("≈") + QString::number(numerical_ans.v,10,7);
        //QString sans = QString::fromStdString(symbolic_ans.toTeX());

        //pCanvas->Refresh(text, sans, nans);
    } catch (const char *s) {
