...
Poly operator * (Poly A, Poly B) {
	Poly C;
//    C.print("x");  out << std::endl;
	for (auto& x : A) for (auto& y : B) {
//	    x.second.print();out << std::endl;
//	    y.second.print();out << std::endl;
	    Rational tmp = C[x.first + y.first] + x.second * y.second;
        C[x.first + y.first] = tmp;
//        tmp.print();out << std::endl;
	}
	C.simp();
	return C;
}
...
void Poly::print(std::ostream &out,const std::string &var) const {
	bool fi = false;
	for (auto &x : *this) {
		if (fi) out << " + "; else fi = true;
		x.second.print(out);
		if (x.first != 0) out << "*" << var << "^" << x.first;
	}
	//out << std::endl;
}
...
Poly Poly::cyclotomic_polynomial(ll n) {
	...
	for (int i = 1; i <= n; i++) if (n % i == 0 && Algo::mu(n / i) == 1) {
		...
		tmp[i] = 1;
		tmp[0] = -1;
//            res.print("x");  out << std::endl;
//            tmp.print("x");  out << std::endl;
		res = res * tmp;
//            res.print("x");  out << std::endl << std::endl;
	}
	for (int i = 1; i <= n; i++) if (n % i == 0 && Algo::mu(n / i) == -1) {
		Poly tmp;
		tmp[i] = 1;
...
void SPoly::print(std::ostream &out,const std::string &var) const {
	bool fi = false;
	for (auto& x : *this) {
		if (fi) out << " + "; else fi = true;
		x.second.print(out);
