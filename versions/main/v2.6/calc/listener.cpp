...
int Listener::Select(QString text,int x,int y){
    int z=0;
    while(x<y-1&&z!=-1){
        if(text[x+1]==QChar('{')){
          z++;
...
void Listener::backspace() {
    ...
    if (pos == -1){
        if(tmp1=="^{"){
            int a=Listener::Select(text,index+3,text.length());
            text.remove(a,2);
    ...
    if(tmp.length()==2){
            ...
            int a=Listener::Select(text,index,text.length());
            text.remove(a,2);
    ...
    if(tmp.length()==5){
        ...
        if(tmp=="\\log{"){
            int a=Listener::Select(text,index,text.length());
            int b=Listener::Select(text,a+2,text.length());
            text.remove(b,2);
            text.replace(a+2,1,"|");
            text.remove(pos,a-pos+2);
        }
        if(tmp=="\\abs{"){
            text.remove(Listener::Select(text,index,text.length()),2);
            text.remove(pos,index-pos);
     }
        else
           text.remove(pos,6);
