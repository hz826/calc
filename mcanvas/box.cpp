...
void Box::insert(int x1,int y1,int y2,int id,bool show){
    w  = qMax(w , x1);
    hu = qMax(hu, qMax(-y1,-y2));
    hd = qMax(hd, qMax( y1, y2));
...
void Box::draw(int x,int y,QPoint *cursor_pos,bool showCursor) {//通过链表绘图
    for (dNode *p = start; p != NULL; p = p->nxt) {
        ...
        switch (p->mode) {
            ...
            case dNode::BOX:
                p->box->draw(p->x+x,p->y+y,cursor_pos,showCursor);
                break;

            case dNode::CURSOR:
                //qDebug() << "size " << cursor.size() << endl;
                if (cursor_pos != NULL)
                    cursor_pos[p->Cursor.id] = QPoint(p->x+x,(p->y+p->Cursor.yy)/2+y);
