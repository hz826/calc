...
Hash PInteger::getHash() const {
    Hash res(0);
    for (int i=n-1;i>=0;i--) {
        res = res * Hash(PInteger::PM) + Hash(a[i]);
    }
    return res;
}
...
Integer::Integer(std::string s) : X(1) {
    if (s[0] == '-') symbol = '-', s = s.substr(1);  else symbol = '+';
    X = PInteger(s);
}
...
Hash Integer::getHash() const {
    if (symbol == '+') return X.getHash();
    else return Hash(0) - X.getHash();
}