...
public:
    ...
    ll encode(const Factor &F) const;
    Factor decode(ll x) const;
    double getDouble(const Factor &F) const;
    void print(std::ostream &out,const Factor &F) const;

    std::string toTeX(const Factor &F) const;
};
